...
let router = new express.Router();
  ...
  router.put("/:id", async function(req, res, next) {
    try {
      ...
      if (balance.rows.length === 0) {
        throw new ExpressError(`No invoice found with the id: ${id}`, 404);
      }
  
      let currPayDate = balance.rows[0].paid_date;
  ...
  router.delete("/:id", async function(req, res, next) {
    try {
      ...
      if (result.rows.length === 0) {
        throw new ExpressError(`The invoice of ${id} was not found`, 404);
      }
